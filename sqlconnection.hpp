...
public:
    SqlConnection(const std::string& databaseFile) : sql(std::make_shared<soci::session>(soci::sqlite3, databaseFile))
    {
...
public:
    UserRepository(std::shared_ptr<soci::session> conn) : connection(conn)
    {
...
public:
    ...
    MessageRepository(std::shared_ptr<soci::session> conn) : connection(conn)
    {
        createMessageTable();
    }
